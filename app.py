...
# ---- Trial screen ----
    ...
    # Dropdown to move between the assigned prompts.
    labels = [f"{i+1}. {PROMPTS_BY_ID[pid]['label']}" for i, pid in enumerate(ss.assigned)]
    choice = st.selectbox("Task", labels, index=ss.idx)
    ss.idx = labels.index(choice)
    ...
    # The single questionnaire, shown once both prompts have been seen.
    st.divider()
    if ss.idx == total - 1:
        ...
        st.info("Please look through both tasks. The questionnaire link appears on the last task.")
